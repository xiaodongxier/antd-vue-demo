...
export default {
  ...
  mounted() {
    // let oldContent = "Hello, world! /n";
    let newContent = "Hi, world!";
    let container = this.$refs.monacoContainer;
    // let oldModel = monaco.editor.createModel(require('./1.txt'), "text/plain");
    import('./1.txt').then(file => {
      const oldModel = monaco.editor.createModel(file, "text/plain");
    });
